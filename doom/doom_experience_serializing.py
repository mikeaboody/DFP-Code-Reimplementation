...
def serialize_experiences_from_mixed_agent(folder, chunk_size, num_steps):
	log_freq = 10000
	doom_simulator = create_basic_simulator(num_simulators)
	goal = np.array([0,0,0,0.5,.5,1])
	possible_actions = enumerate_action_one_hots(3)
	...
	while i < num_steps:
		...
		for simul_i in range(num_simulators):
			...
			if i % log_freq == 0:
				#log it
				with open("mixed_serialization_progress.txt", "a") as myfile:
					myfile.write(str(i) + "\n")
			if (terminated[simul_i]):
				exp_creator.reset()
			else:
	...
	exp_serializer.flush()
	doom_simulator.close_game()
serialize_experiences_from_mixed_agent("mixed_exp", 250, 5000)
